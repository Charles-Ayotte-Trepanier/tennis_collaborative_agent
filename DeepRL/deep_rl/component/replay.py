...
Transition = namedtuple('Transition', ['state', 'colleague_state', 'action', 'reward', 'next_state', 'mask', 'next_colleague_state'])
PrioritizedTransition = namedtuple('Transition',
                                   ['state', 'colleague_state', 'action',
                                    'reward', 'next_state', 'mask',
                                    'sampling_prob', 'idx',
                                    'next_colleague_state'])
...
class Storage:
    def __init__(self, memory_size, keys=None):
        ...
        keys = keys + ['state', 'action', 'reward', 'mask',
                       ...
                       'mean', 'next_state', 'colleague_state',
                       'next_colleague_state']
        self.keys = keys
...
class UniformReplay(Storage):
    ...
    def construct_transition(self, index):
        ...
        colleague_state = [self.colleague_state[i] for i in range(s_start, s_end + 1)]
        next_colleague_state = [self.next_colleague_state[i] for i in range(next_s_start, next_s_end + 1)]
        if self.history_length == 1:
            # eliminate the extra dimension if no frame stack
            state = state[0]
            next_state = next_state[0]
            colleague_state = colleague_state[0]
            next_colleague_state = next_colleague_state[0]
        state = np.array(state)
        next_state = np.array(next_state)
        colleague_state = np.array(colleague_state)
        next_colleague_state = np.array(next_colleague_state)
        cum_r = 0
        cum_mask = 1
